...
# 🔍 Ключевые слова (ваш полный список)
KEYWORDS = [
    ...
    for src in SOURCES:
        try:
            ...
            if resp.status_code == 403:
                log.warning(f"HTTP 403 from {src['name']}")
                continue
    ...
    for channel_id in CHANNEL_IDS:
        ...
        try:
            ...
            if r.status_code == 200:
                log.info(f"✅ Отправлено в {channel_id}")
            else:
                log.error(f"❌ Ошибка в {channel_id}: {r.text}")
                success = False
        except Exception as e:
            log.error(f"❌ Исключение в {channel_id}: {e}")
            success = False
    return success
...
if __name__ == "__main__":
    ...
    while True:
        messages = fetch_all_relevant_news()
        count = 0
